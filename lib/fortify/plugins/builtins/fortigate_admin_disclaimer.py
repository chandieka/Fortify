...
class AdminDisclaimerPlugin(FortifyPlugin):
    _name = "admin_disclaimer"
    _option_requirement = dict()
    _description = "this plugin check if the device has enabled administrator disclaimer"

    def action(self):
        super(AdminDisclaimerPlugin, self).action()
        ...
        if global_config.get("pre-login-banner") == "enable":
            fpu.success_plugin(
                msg="Administrator disclaimer banner is enabled"
            )
        else:
            fpu.fail_plugin(
                msg="Administrator disclaimer banner is not enabled"
            )
